...
| ----------- |:------------|
| Account | The account used to connect a publisher to a message bus |
...
| Node | A node is a device or service that provides information and accepts control input. Information from this node can be published by the node itself or published by a (proxy) service that knows how to access the node. 
...
| Publisher| A service node that is responsible for publishing information (node outputs) and discovery, and handle configuration updates and control inputs. Publishers can include identification with published information to provide transparency of the data source.
...
characters. Reserved address keywords start with a dollar ($) character.
...
|----------------|-----------|
|{zoneId} | The zone in which publishing takes place. The local and default zoneId is ‘myzone’. This is only valid within a zone and akin to localhost on a computer. See bridging for more information on connecting multiple zones.
...
| Address segment  | Description|
|:---------------- | -----------
| {zoneId} | The zone in which discovery takes place. The default zoneId is **$myzone** which indicates the current local zone.
...
| Address segment | Description
|-----------------|--------------
| {zoneId}        | The zone in which discovery takes place. The default zoneId is ‘myzone’
